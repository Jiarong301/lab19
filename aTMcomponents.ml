...
Database module. That module's interface can be found in database.mli,
and provides lower-level manipulations of the database independent of
any partcular concrete OCaml data structure implementating the
database. To demonstrate the implementation-independence made possible
by the abstraction barrier from the module's signature, we provide
three different implementations of the database module in database.ml,
database2.ml, and database3.ml. (See those files for further
information.)

Because the signature for ATMcomponents returns unit for many
functions that implicitly modify the database (initialize and update
in particular), the implementation of the database from within
ATMcomponents must allow for modification by side effect. A purely
functional implementation is not possible. We pass that requirement on
to the Database module as well. Its interface also is imperative
rather than functional. An alternative would be to have a functional
database module and do all of the storing and side effects within
ATMcomponents.
